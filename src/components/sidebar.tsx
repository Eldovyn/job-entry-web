...
const SideBar: React.FC = () => {
    ...
    return (
        ...
        >
            {isSm ? (
                <Image src={IconProject} alt="Logo" className={`w-[45px] mt-5 ${isSm && 'ms-4'}`} />
            ) : (
                <>
            ...
            <ul className="text-white mt-3 ms-1 overflow-hidden">
                <li>
                    <Link
                        href="#"
                        className={`flex items-center gap-3 py-3.5 px-3 md:px-5 cursor-pointer rounded-sm ${
                            isActive('/dasbor-admin')
                                ? 'border border-primary bg-red-700'
                                : ''
                        }`}
                    >
                        <IoHomeOutline size={22} className={`${isSm && 'ms-3'}`} />
                        {!isCollapsed && (
                ...
                <li>
                    ...
                    >
                        <VscAccount size={22} className={`${isSm && 'ms-3'}`} />
                        {!isCollapsed && (
